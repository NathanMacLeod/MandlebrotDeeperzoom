...
static float get_iterations_to_diverge(cmplx<float_type> x_0, int max_itr, bool smoothed = true) {
	int itr = 0;
...
template<class float_type>
static float continue_iterations_to_diverge(cmplx<float_type> x_i, cmplx<float_type> x_0, int max_itr, bool smoothed = true) {
	int itr = 0;
	while (itr < max_itr && x_i.mag_sqr() < 4) {
		itr++;
		x_i = x_i * x_i + x_0;
...
static float get_smoothed_escape_radius(double escape_radius, cmplx<double> first_escaped_pos, int itr_to_escape) {
...
template<int mantissa_len>
static std::vector<float> approx_mandlebrot_itrerations_of_grid(int screen_width, int screen_height, int max_itr, int min_pixel_x, int min_pixel_y, int grid_width, int grid_height, double inv_zoom, cmplx<BigFloat<mantissa_len>> camera_coord, bool smoothed = true) {
